...
## Making a function for making pcture from json
...
def make_picture(training_data_filename, model, new_inp_np_arr, output_file):
    ...
    fig = px.scatter(x=ages, y=height, title="Height vs Ages", labels={'x': 'Age (years)', 'y': 'Height (inches)'})
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='markers', 
        marker=dict(
            color='purple', size=20, line=dict(
                color='purple',
                width=2
            )
        ) 
